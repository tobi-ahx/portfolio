  ...
  return (
    ...
    initial={{opacity: 0}} whileInView={{opacity: 1}} transition={{duration: 1.5}} viewport={{once: true}}>
        ...
        <form action={async (formData) => {
            ...
            <input name="senderEmail" className='h-14 rounded-lg border border-black/[0.1] px-4' type="email" placeholder="Your E-Mail" required maxLength={500}/>
            <textarea name="message" className='h-52 my-3 rounded-lg border border-black/[0.1] p-4' placeholder="Your message" required maxLength={5000}></textarea>
            <SubmitBtn />
        </form>
    </motion.section>
  )
